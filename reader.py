...
class Reader:
    ...
    def process_dataset(self, *row_parts):
        ...
        if not self.is_evaluating and self.config.RANDOM_CONTEXTS:
            ...
            rand_indices = tf.random_shuffle(tf.range(safe_limit))[
        ...
        sparse_split_source = tf.sparse.SparseTensor(
            ...
            dense_shape=[
                self.config.MAX_CONTEXTS,
                tf.maximum(
                    tf.to_int64(self.config.MAX_NAME_PARTS), split_source.dense_shape[1]
                ),
            ],
        )
        ...
        sparse_split_target = tf.sparse.SparseTensor(
            ...
            dense_shape=[
                self.config.MAX_CONTEXTS,
                tf.maximum(
                    tf.to_int64(self.config.MAX_NAME_PARTS), split_target.dense_shape[1]
                ),
            ],
        )
...
if __name__ == "__main__":
    ...
    config = Config()
    reader = Reader(
        subtoken_to_index, target_word_to_index, node_to_index, config, False
    )
    ...
    try:
        while True:
            target_indices, target_strings, target_lengths, path_source_indices, node_indices, path_target_indices, valid_context_mask, path_source_lengths, path_lengths, path_target_lengths, path_source_strings, path_strings, path_target_strings = sess.run(
                [
                    target_index_op,
                    target_string_op,
                    target_length_op,
                    path_source_indices_op,
                    node_indices_op,
                    path_target_indices_op,
                    valid_context_mask_op,
                    path_source_lengths_op,
                    path_lengths_op,
                    path_target_lengths_op,
                    path_source_strings_op,
                    path_strings_op,
            ...
            print(
                ...
                Common.binary_to_string_3d(
                    np.concatenate(
                        [path_source_strings, path_strings, path_target_strings], -1
                    )
                ),
            )
            ...
            print(
                "Path source strings: ", Common.binary_to_string_3d(path_source_strings)
            )
            ...
            print(
                "Path target strings: ", Common.binary_to_string_3d(path_target_strings)
            )
